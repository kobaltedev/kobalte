...
export default function App() {
  ...
  const [filteredOptions, setFilteredOptions] = createSignal(optionList);

  const { contains } = createFilter({ sensitivity: "base" });

  const onInputChange = (value: string) => {
    setQuery(value);
    setFilteredOptions(optionList.filter(item => contains(item.name, value)));
  };

  const onOpenChange = (isOpen: boolean, triggerMode?: string) => {
    ...
    <I18nProvider locale="en-US">
      <form style={{ display: "flex" }} ref={formRef} onSubmit={onSubmit}>
        <Combobox.Root
          ...
          displayValue={value => optionList.find(option => option.code === value)?.name ?? ""}
          ...
          <MultiCombobox.Control class="combobox__control">
            {({ values }) => (
              <>
                <span>
                  {optionList
                    .filter(option => values().includes(option.code))
