...
import {
  CalendarGridCellContext,
  CalendarGridCellContextValue,
} from "./calendar-grid-cell-context";
import { getEraFormat } from "./utils";

export interface CalendarGridCellOptions {
 ...
 */
export const CalendarGridCell = createPolymorphicComponent<"td", CalendarGridCellOptions>(props => {
  const [local, others] = splitProps(props, ["as", "date", "isDisabled"]);

  return (
    <Show when={local.date} fallback={<td {...others} />}>
      <CalendarGridCellBase date={local.date!} isDisabled={local.isDisabled} {...others} />
    </Show>
  );
});

interface CalendarGridCellBaseOptions {
  date: CalendarDate;
  isDisabled?: boolean;
}

const CalendarGridCellBase = createPolymorphicComponent<"td", CalendarGridCellBaseOptions>(
  props => {
    ...
    const context: CalendarGridCellContextValue = {
      isSelectable,
      isDisabled,
      isUnavailable,
      isSelected,
    ...
    return (
      <CalendarGridCellContext.Provider value={context}>
        <Dynamic
          component={local.as}
          role="gridcell"
          ...
          {...others}
        />
      </CalendarGridCellContext.Provider>
    );
  }
);
