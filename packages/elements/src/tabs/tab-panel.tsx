 ...
 */
export const TabPanel = createPolymorphicComponent<"div", TabPanelProps>(props => {
  let ref!: HTMLElement;
  ...
  const id = () => local.id ?? context.generateTabPanelId(local.value);
  ...
  createEffect(
    on([() => local.value, id], ([value, id]) => {
      context.tabPanelIdsMap().set(value, id);
    })
  );
