...
The `@kobalte/core` package provides a set of low-level UI components and primitives which can act as a base layer for your design system implementation.
...
Kobalte provides granular access to each component parts, so you can wrap them and add your own event listeners, props, or refs.
