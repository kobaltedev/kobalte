...
Some Kobalte components normalizes press interactions (mouse, touch, keyboard, and screen readers) behavior across browsers and platforms, and handles many nuances of dealing with pointer and keyboard events.
...
Press down on the button and drag your pointer off and over to see when the events are fired, and try focusing the button with a keyboard and pressing the <Kbd>Enter</Kbd> or <Kbd>Space</Kbd> keys to trigger events as well.
