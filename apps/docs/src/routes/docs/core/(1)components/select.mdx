...
badges:
  - icon: npm
    label: "Since v0.1.0"
  - label: "@kobalte/core/select"
  - icon: source
    label: "Source"
    url: "https://github.com/kobaltedev/kobalte/tree/main/packages/core/src/select"
  - icon: aria
    label: "Listbox"
...
```tsx title="index.tsx" tab file="../../../../examples/select.tsx#L1-L40"
```

```css title="style.css" tab file="../../../../examples/select.module.css"
```
...
`Select.Label`, `Select.Trigger`, `Select.Value`, `Select.Description` and `Select.ErrorMessage` shares the same data-attributes.
