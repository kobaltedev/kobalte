...
- **Pagination.List:** Contains the list of items and allows a user to select one of them.
...
The appearance can be customized by hiding the First and Last item.
...
The appearance can be customized by changing the items around the current page item.
