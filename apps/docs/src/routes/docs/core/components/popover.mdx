...
<TabsSnippets>
  ...
  <TabsSnippets.Content value="style.css">
    ...
    .popover__trigger[data-hover] {
      background-color: hsl(201 96% 32%);
    }

    .popover__trigger[data-focus-visible] {
      outline: 2px solid hsl(200 98% 39%);
      outline-offset: 2px;
    }

    .popover__trigger[data-active] {
...
```tsx
<Popover.Root defaultIsOpen>...</Popover.Root>
```
...
| isModal       | `boolean` <br/> Whether the popover should be the only visible content for screen readers, when set to `true`: <br/> - interaction with outside elements will be disabled. <br/> - scroll will be locked. <br/> - focus will be locked inside the popover content. <br/> - elements outside the popover content will not be visible for screen readers. |
