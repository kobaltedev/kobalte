...
The `value` prop, which accepts a value corresponding with the `value` prop of each radio, can be used to make the value controlled. The `onValueChange` event is fired when the user selects a radio, and receives the new value.
...
| data-active        | Present when the radio is activated by a pointer down (mouse, touch, pen, etc...). |
