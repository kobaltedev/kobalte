...
<TabsSnippets>
  ...
  <TabsSnippets.Content value="style.css">
    ...
    .hovercard__trigger[data-hover] {
      text-decoration: underline;
    }

    .hovercard__trigger[data-focus-visible] {
...
```tsx
<HoverCard.Root defaultIsOpen>...</HoverCard.Root>
```

### Controlled open

The `isOpen` prop can be used to make the open state controlled. The `onOpenChange` event is fired when the user pointer enter or leave the trigger, and receives the new value.
...
function ControlledExample() {
  ...
  return (
    <HoverCard.Root isOpen={open()} onOpenChange={setOpen}>
      ...
    </HoverCard.Root>
  );
}
