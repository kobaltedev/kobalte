...
The `value` prop, which accepts a value corresponding with the `options` prop, can be used to make the value controlled. The `onChange` event is fired when the user selects an item, and receives the new value.
...
The `MultiSelect.*` components can be used to create a select that allow multi-selection. In this case the value provided to `value`, `defaultValue` and `onChange` props are of type `Set<string>` instead of `string`.
...
| onChange         | `(value: Set<string>) => void` <br/> Event handler called when the value changes.                                                               |
