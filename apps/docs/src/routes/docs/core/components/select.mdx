...
Additionally, the `renderSection` prop is used to display the option group label in the select.
...
export function OptionGroupExample() {
  return (
    <Select.Root<Food, Category>
      ...
      renderSection={section => <Select.Section>{section().rawValue.label}</Select.Section>}
    >
      <Select.Trigger aria-label="Food">
        <Select.Value />
        <Select.Icon>
...
The `MultiSelect.*` components can be used to create a select that allow multi-selection. In this case the value provided to `value`, `defaultValue` and `onValueChange` props are of type `Set<string>` instead of `string`, and the `renderValue` prop expose an array of selected options instead of a single one.
...
| renderValue                   | `(selectedOption: Accessor<T>) => JSX.Element` <br/> A map function that receives a _selectedOption_ signal representing the selected option.                                                                                                                                                                                                        |
...
| renderItem                    | `(item: Accessor<CollectionNode<T>>) => JSX.Element` <br/> When NOT virtualized, a map function that receives an _item_ signal representing an item.                                                                                                                                                                                                 |
| renderSection                 | `(section: Accessor<CollectionNode<U>>) => JSX.Element` <br/> When NOT virtualized, a map function that receives a _section_ signal representing a section.                                                                                                                                                                                          |
...
| renderValue   | `(selectedOptions: Accessor<T[]>) => JSX.Element` <br/> A map function that receives a _selectedOptions_ signal representing the selected options. |
