...
export const Popover = () => {
	return (
		<KPopover>
			...
			<KPopover.Content class="popover__content">...</KPopover.Content>
		</KPopover>
	);
};
```
...
If you are using [TailwindCSS v3](https://v3.tailwindcss.com/), you can use the `@kobalte/tailwindcss` plugin to target Kobalte's `data-*` attributes with modifiers like `ui-expanded:*`. From Tailwind v4 the plugin is no longer required as data attributes are natively supported, use them directly instead.
...
Follow the [Polymorphism Guide](./polymorphism) for more advanced type control and additional props.
