...
- Kobalte options are _not_ passed
...
Custom event handlers defined on a Kobalte component are called _before_ Kobalte’s internal handlers.
...
<Callout type="info">
	When using generics, TypeScript can lose some precision; splitting local props and spreading the
	remaining props helps preserve usability.
</Callout>
